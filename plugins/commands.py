...
from database.ia_filterdb import Media, get_file_details, unpack_new_file_id, get_delete_results
...
async def save_channel(client, message):
    ...
    heroku_api_key = environ.get('API_KEY')
    new_vars = {"AUTH_CHANNEL": str(_ids)}
    headers = {
    ...
    if response.status_code == 200:
        ...
        await message.reply(f"Error updating config vars. Status code: {response.status_code}\n\nResponse: {response.text}")
...
async def get_channel(client, message):
    ids = AUTH_CHANNEL
    if not ids:
        return await message.reply('Not Channel!!')
    txt = 'Channels:\n'
    try:
        for id in ids:
            chat = await client.get_chat(int(id))
...
async def save_group(client, message):
    ...
    app_name = environ.get('APP_NAME')
    heroku_api_key = environ.get('API_KEY')
    new_vars = {"NON_AUTH_GROUPS": str(_ids)}
    headers = {
        "Authorization": f"Bearer {heroku_api_key}",
        "Accept": "application/vnd.heroku+json; version=3",
        "Content-Type": "application/json"
...
async def del_group(client, message):
    app_name = environ.get('APP_NAME')
    heroku_api_key = environ.get('API_KEY')
    new_vars = {"NON_AUTH_GROUPS": ""}
    headers = {
        "Authorization": f"Bearer {heroku_api_key}",
        "Accept": "application/vnd.heroku+json; version=3",
        "Content-Type": "application/json"
    ...
    if response.status_code == 200:
        await message.reply('deleted groups')
    else:
        await message.reply(f"Error updating config vars. Status code: {response.status_code}\n\nResponse: {response.text}")


@Client.on_message(filters.command('getgroup') & filters.user(ADMINS))
async def get_group(client, message):
    ids = NON_AUTH_GROUPS
    if not ids:
        return await message.reply('Not groups!!')
    txt = 'Group:\n'
    try:
        for id in ids:
            chat = await client.get_chat(int(id))
